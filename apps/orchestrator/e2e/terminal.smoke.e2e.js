...
async function main() {
  ...
  try {
    ...
    if (!result || result.error) {
      throw new Error(result && result.error ? result.error : "terminal_smoke failed");
    }
    console.log("[e2e] terminal_smoke ok");
  } finally {
    if (client) {
      await client.quit();
    }
