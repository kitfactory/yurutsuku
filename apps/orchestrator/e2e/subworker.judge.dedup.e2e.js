...
async function openAndSetIpcSessionId(client) {
  ...
  if (!sessionId) throw new Error("ipc session id not ready");
...
function resetDebugFile(filePath) {
  ...
  try {
    ...
    // ignore missing file / 未作成ファイルは無視
  }
}
  ...
  try {
    ...
    for (const line of lines) {
      try {
        ...
        // ignore malformed lines / 壊れた行は無視
      }
    }
    return entries;
  } catch {
...
function extractEvents(entries, sessionId) {
  return entries.filter((entry) => entry && entry.session_id === sessionId);
}
...
async function main() {
  ...
  // Deterministic subworker LLM response / サブワーカー応答を固定化する
  process.env.NAGOMI_TOOL_PATH = process.execPath;
  process.env.NAGOMI_SUBWORKER_TOOL_ARGS = path.join(__dirname, "stubs", "subworker_llm_stub.js");
  ...
  try {
    ...
    if (!previousSettings) throw new Error("settings not loaded");
    ...
    const internalBefore = await client.executeScript(
      "return window.nagomiTest.getInternalState();"
    );
    const terminalSessionId = String(internalBefore.terminalSessionId || "");
    if (!terminalSessionId) throw new Error("terminal session id missing");
    ...
    if (!debugFile) throw new Error("subworker debug file path not returned");
    resetDebugFile(debugFile);

    // Prepare deterministic terminal tail + input. / 文脈を固定化して重複判定を安定させる。
    await client.executeScript(`
      outputTailBuffer = "Pick one: 1 or 2\\n> ";
      lastInputLine = "前回はグーでした。次はどれ？";
      if (typeof registerTestHooks === 'function') registerTestHooks();
    `);

    // Same logical completion from different sources must dedup. / source違いでも同一完了は1回に畳む。
    const triggered = await client.executeScript(`
      if (!window.nagomiTest || !window.nagomiTest.triggerSubworkerJudgeComplete) return false;
      window.nagomiTest.triggerSubworkerJudgeComplete({
        state: 'need-input',
        reason: 'dedup-same-context',
        source: 'hook-judge',
      });
      window.nagomiTest.triggerSubworkerJudgeComplete({
        state: 'need-input',
      ...
      return true;
    `);
    if (!triggered) throw new Error("failed to trigger judge-complete events");

    await waitFor(async () => {
      const events = extractEvents(readJsonl(debugFile), terminalSessionId);
      return events.some((e) => e.event_type === "llm-start");
    }, 20000);

    await waitFor(async () => {
      const events = extractEvents(readJsonl(debugFile), terminalSessionId);
      return events.some((e) => e.event_type === "skip-judge-complete-dedup");
    }, 20000);

    const events = extractEvents(readJsonl(debugFile), terminalSessionId);
    const llmStarts = events.filter((e) => e.event_type === "llm-start");
    const dedupSkips = events.filter((e) => e.event_type === "skip-judge-complete-dedup");
    if (llmStarts.length !== 1) {
      throw new Error(`expected exactly 1 llm-start, got ${llmStarts.length}`);
    }
    if (dedupSkips.length < 1) {
      throw new Error("expected skip-judge-complete-dedup event");
    }
    const dedupDetail = (dedupSkips[dedupSkips.length - 1] || {}).details || {};
    if (!dedupDetail.dedup_signature) {
      throw new Error("dedup skip event missing dedup_signature");
    }

    // Non-Tab key should dismiss ghost suggestion without applying it.
    // Tab以外キーはghost提案を解除し、提案適用はしない。
    await waitFor(async () => {
      const snapshot = await client.executeScript("return window.nagomiTest.getInternalState();");
      return Boolean(snapshot && snapshot.subworker && snapshot.subworker.suggestedInput);
    }, 20000);
    ...
    const eventsAfterTyping = extractEvents(readJsonl(debugFile), terminalSessionId);
    const accepted = eventsAfterTyping.filter((e) => e.event_type === "accept-suggestion");
    if (accepted.length !== 0) {
      throw new Error("non-Tab key unexpectedly accepted suggestion");
    }
    const clearedByTyping = eventsAfterTyping.some(
      (e) =>
        e.event_type === "clear-suggestion" &&
        e.details &&
        String(e.details.reason || "") === "user-typing"
    );
    if (!clearedByTyping) {
      throw new Error("suggestion was not cleared by typing");
    }

    console.log("[e2e] subworker judge dedup + ghost non-tab OK", {
      terminalSessionId,
      llmStartCount: llmStarts.length,
      dedupSkipCount: dedupSkips.length,
    });
  } finally {
    if (client) {
      try {
        ...
        // ignore cleanup errors / 終了処理エラーは無視
      }
    }
    driver.kill();
  }
