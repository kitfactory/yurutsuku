...
- [x] K-7 テスト: `node apps/orchestrator/e2e/terminal.tint.e2e.js` が通る
  - 前提不足時は skip を返すため、必ず fail させる検証は `NAGOMI_E2E_STRICT=1` で実行する
...
- [x] L-3 仕様/設計/実装: `idle/success/failure -> need_input` の直行を禁止し、`running` 経由を強制する遷移ガードを追加する
- [x] L-3 テスト: `npm test -w apps/orchestrator -- --test-reporter=spec` と `NAGOMI_E2E_STRICT=1 node apps/orchestrator/e2e/terminal.tint.e2e.js` で回帰がない
