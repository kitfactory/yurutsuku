...
### N-10 AI 状態判定（hook-only）
- [x] N-10.1 文書: 状態遷移と色対応（`idle/success=黒`、`need_input=オレンジ`、`failure=赤`、`subworker-running=緑オーバーレイ`）を `docs/concept.md` / `docs/spec.md` / `docs/architecture.md` / `docs/tauri-driver-e2e.md` に反映する
- [x] N-10.1 テスト: docs の記述が一意で矛盾しない（自己レビュー）
- [x] N-10.2 実装: 観測イベント→状態遷移→tint/watcher 反映の経路を整理し、遷移漏れ/色反映漏れを防ぐ
- [x] N-10.2 テスト: `cargo test -p nagomi-orchestrator` / `npm test -w apps/orchestrator -- --test-reporter=spec` / 目視E2Eで `hook-complete(success|need_input|failure)` が色と一致することを確認する
...
- [x] N-10.4 テスト: 入力送信では状態が変わらず、`hook completed` だけで黒/オレンジ/赤へ遷移することを実測で確認する
- [x] N-10.6 テスト: `codex` 起動（コマンドのみ）でも初回指示送信後でも `idle` を維持し、hook 到達時だけ状態が変わることを E2E で検証する（`apps/orchestrator/e2e/codex.prime-minister.e2e.js`）
- [x] N-10.5 実装: IME 変換中（`isComposing` / `keyCode=229`）の Enter は入力確定扱いにせず、入力途中で不要なフック外処理が走らないようにする
- [x] N-10.5 テスト: `npm test -w apps/orchestrator` で IME ガード（統合テスト）が通ることを確認する
- [x] N-10.7 実装: Codex / ClaudeCode の状態確定を hook-only に整理し、terminal idle/exit fallback 設定を撤去する
- [x] N-10.7 テスト: `cargo test --manifest-path apps/orchestrator/src-tauri/Cargo.toml rewrite_codex_notify_config -- --nocapture` と `cargo check -p nagomi-orchestrator`、hook 実測で `codex.jsonl` 取得を確認する
- [x] N-10.8 実装: hook 完了時の `input_messages` / `last_assistant_message` / `cwd` を project ごとの JSONL（`project-prompt-history/<project-key>.jsonl`）へ保存し、後からプロンプト改善に使えるようにする
- [x] N-10.8 テスト: `npm test -w apps/orchestrator -- --test-reporter=spec` と `cargo test --manifest-path apps/orchestrator/src-tauri/Cargo.toml build_project_prompt_history_record_extracts_hook_io_by_project project_prompt_history_key_is_stable_for_same_locator -- --nocapture`、`cargo check -p nagomi-orchestrator` で回帰がないことを確認する
- [x] N-10.9 実装: `nagomi prompt-history export` を追加し、project ごとの hook 入出力履歴を `--project` で絞り込みつつ `json/jsonl` で stdout または `--output` へ書き出せるようにする
- [x] N-10.9 テスト: `npm test -w apps/orchestrator -- --test-reporter=spec` で CLI の export 統合テスト（stdout / `--output`）が通ることを確認する
- [x] N-10.10 文書/実装: 状態管理を `hook-complete -> success/need_input/failure` に統一し、入力確定/プロセス開始/終了の状態遷移と AI 状態判定トグルを撤去する
- [x] N-10.10 テスト: `npm test -w apps/orchestrator -- --test-reporter=spec` / `cargo check -p nagomi-orchestrator` / `cargo test --manifest-path apps/orchestrator/src-tauri/Cargo.toml settings_roundtrip build_project_prompt_history_record_extracts_hook_io_by_project -- --nocapture` で hook-only 経路の回帰がないことを確認する
...
- [x] N-12.12 文書: サブワーカーの起動条件を「状態遷移時」から「hook 完了時」へ更新し、`ガンガン` の `failure` 完了時は待機する仕様を `docs/OVERVIEW.md` / `docs/spec.md` に反映する
- [x] N-12.12 実装: サブワーカー起動トリガーを `state-update` から `hook 完了` に変更し、`ガンガン` の `failure` 完了時は起動しないようにする
- [x] N-12.12 テスト: `subworker_ui_and_settings`（統合）と subworker 関連の選択実行テストで新トリガー（hook-only）を確認し、E2E（terminal/tint）で表示回帰がないことを確認する
...
- [x] N-12.14 文書: 仮想ケース（S1 慎重+アドバイス / S2 慎重+代理入力 / S3 ガンガン+fail待機 / S4 アドバイス専用）に相当するケースを `apps/orchestrator/e2e/subworker.matrix.e2e.js` に定義し、順序アサート（hook 完了前禁止・完了後起動）を明記する
...
- [x] N-12.15 文書: トリガー統合方針を hook-only に更新し、サブワーカー起動は hook 完了イベントのみとすることを `docs/OVERVIEW.md` / `docs/spec.md` / `docs/architecture.md` に明記する
- [x] N-12.15 実装: codex prompt marker / terminal idle / terminal exit を AI状態確定ソースから外し、hook 完了イベントへ収束させる
- [x] N-12.15 テスト: `codex_hook_only_state_flow` と subworker 関連統合テストで、hook 経路から状態確定と起動条件が hook 限定であることを確認する
...
- [x] N-12.95 実装: 設定画面をカテゴリタブ（`外観 / Windows / AI / 通知 / キャラクター`）へ整理し、縦長でも全体像を予測しやすくする
- [x] N-12.95 テスト/文書: `npm test -w apps/orchestrator -- --test-reporter=spec` と `cargo build -p nagomi-orchestrator`、`nagomi --help` / `nagomi --status` で回帰確認し、`docs/OVERVIEW.md` / `docs/spec.md` / `docs/architecture.md` を同期する
- [x] N-12.96 実装: Windows の通常 watcher に `shadow=false` と DWM 属性を適用し、透明 3D ウィンドウの影/境界線/角丸を抑える
- [x] N-12.96 テスト/文書: `cargo build -p nagomi-orchestrator`、`npm test -w apps/orchestrator -- --test-reporter=spec`、`nagomi --help` / `nagomi --status` で回帰確認し、`docs/OVERVIEW.md` / `docs/spec.md` / `docs/architecture.md` を同期する
- [x] N-12.97 実装: 通常 watcher の native titlebar 切替をやめ、選択時だけ custom frame の `閉じる` / `リサイズ` を出す
- [x] N-12.97 テスト/文書: `cargo build -p nagomi-orchestrator`、`npm test -w apps/orchestrator -- --test-reporter=spec`、`nagomi --help` / `nagomi --status` で回帰確認し、`docs/OVERVIEW.md` / `docs/spec.md` / `docs/architecture.md` を同期する
- [x] N-12.98 実装: 通常 watcher の custom frame をやめ、Windows の native titlebar / 枠で移動・最大化・閉じる・サイズ変更できるよう戻す
- [x] N-12.98 テスト/文書: `npm test -w apps/orchestrator -- --test-reporter=spec` と `cargo build -p nagomi-orchestrator` で回帰確認し、`docs/OVERVIEW.md` / `docs/spec.md` / `docs/architecture.md` を同期する
- [x] N-12.99 実装: 通常 watcher の native frame は常時表示ではなく、focus 中のみ表示して blur で外す
- [x] N-12.99 テスト/文書: `npm test -w apps/orchestrator -- --test-reporter=spec` と `cargo build -p nagomi-orchestrator` で回帰確認し、`docs/OVERVIEW.md` / `docs/spec.md` / `docs/architecture.md` を同期する
- [x] N-12.90 文書/実装: terminal 終了 cleanup を backend 正本へ寄せ、native window close から session/worker cleanup と `--exit-on-last-terminal` を完了できるようにする
- [x] N-12.90 テスト: `cargo build -p nagomi-orchestrator` / `npm test -w apps/orchestrator -- --test-reporter=spec` と `nagomi --status` で回帰がないことを確認する
- [x] N-12.91 文書/実装: Settings > AI Coding Agent のサブワーカー用プロンプト編集を inline textarea から簡易エディタ modal に置き換え、カード側はプレビュー + `編集` / `既定に戻す` に整理する
- [x] N-12.91 テスト: `npm test -w apps/orchestrator -- --test-reporter=spec` の統合文字列検証と `cargo build -p nagomi-orchestrator`、`nagomi --status` で回帰がないことを確認する
- [x] N-12.92 文書/実装: Windows terminal shell 設定に `PowerShell 7`（`pwsh`）を追加し、`pwsh.exe` がある環境でのみ UI に表示する。未導入時の stale 設定は backend で `powershell.exe` へフォールバックする
- [x] N-12.92 テスト: `cargo build -p nagomi-orchestrator` / `npm test -w apps/orchestrator -- --test-reporter=spec` / `nagomi --status` で回帰がないことを確認する
...
- [x] N-12.21 実装: `running` / `ai-running` を状態表示から撤去し、`subworker-running` のみを補助ステータスとして残す
- [x] N-12.21 テスト: `npm test -w apps/orchestrator -- --test-reporter=spec` で `running` 撤去後の統合テスト回帰が通ることを確認する
- [x] N-12.22 文書: サブワーカー起動トリガーを論理イベント `hook-complete` に統一し、`need_input` 放置禁止（`start` または `skip(理由付き)` 必須）を `docs/OVERVIEW.md` / `docs/concept.md` / `docs/spec.md` / `docs/architecture.md` に反映する
- [x] N-12.22 テスト: docs の記述が一意で矛盾しない（自己レビュー）
- [x] N-12.23 実装: `queueSubworkerOnHookComplete` の起動条件を `hook-complete` 正規化へ置換し、hook 完了ソースで同一経路処理する
- [x] N-12.23 テスト: hook 経路で `need_input` 時に `start` または `skip(理由)` が `subworker_debug_events.jsonl` に必ず記録されることを確認する（`npm test -w apps/orchestrator -- --test-reporter=spec` / `npm run -w apps/orchestrator e2e:subworker:matrix`）
- [x] N-12.24 実装: 状態デバッグログ（`status_debug_enabled`）を追加し、状態遷移/フック/サブワーカーイベントを `status_debug_events.jsonl` に自動記録できるようにする
...
- [x] N-12.29.1 文書: サブワーカー実行フローを「hook-complete を唯一の起点」として整理し、`1判定=1回実行`（同一文脈の重複起動抑止）と ghost 入力仕様（`Tab`=補完確定、`Tab`以外=ghost解除して通常入力）を `docs/OVERVIEW.md` / `docs/spec.md` / `docs/architecture.md` に明記する
- [x] N-12.29.2 実装: `queueSubworkerOnHookComplete` の dedup 署名を source 非依存にし、同一文脈の `hook-complete` 連発を短時間で1回に畳む
...
- [x] N-12.29.5 テスト: `npm test -w apps/orchestrator` と `npm run e2e:smoke -w apps/orchestrator` を実行し、`npm run e2e:subworker:judge:dedup -w apps/orchestrator` のログで `hook-complete` 1回に対して `llm-start` が重複しないこと（`llmStartCount: 1`）を確認する
...
- [x] N-12.45 文書: `Esc` 手動介入ターン（`manual-hold`）を仕様化し、`Esc` で hook/SubWorker を停止、`Enter` 確定まで再起動しない条件を `docs/OVERVIEW.md` / `docs/spec.md` / `docs/architecture.md` に反映する
- [x] N-12.45 実装: `apps/orchestrator/src/index.html` に `manual-hold` 状態を追加し、hook/SubWorker の起動経路（hook / hook-complete）を `Esc` で停止、`Enter` で解除する
...
- [x] N-12.46 テスト: `npm test -w apps/orchestrator -- --test-reporter=spec` で統合テスト回帰を確認し、`pty-output` 非破壊クリアの実装文字列検証を追加する
- [x] N-12.47 実装: Codex 処理中（`agentWorkActive`）は補完プレースホルダ描画を抑止し、本文出力への割り込みを防ぐ
- [x] N-12.47 テスト: `npm test -w apps/orchestrator -- --test-reporter=spec` で統合テスト回帰を確認し、処理中プレースホルダ抑止の実装文字列検証を追加する
...
- [x] N-12.82 実装: ターミナルキャラクター表示を既定 ON にし、通常 watcher の renderer 切替 UI を撤去して 3D 専用へ簡素化する
- [x] N-12.82 テスト/文書: `apps/orchestrator/integration.test.js` の 3D固定前提を更新し、`docs/OVERVIEW.md` / `docs/concept.md` / `docs/spec.md` / `docs/architecture.md` を「既定ON + 通常 watcher は常時3D」へ同期する
- [x] N-12.83 実装: Terminal / Settings の右クリックメニューに `Open New Terminal` を追加し、選択中 terminal または現在 window を基準に少し右下へずらして新規 terminal を開けるようにする
- [x] N-12.83 テスト/文書: `apps/orchestrator/integration.test.js` と `docs/OVERVIEW.md` / `docs/spec.md` / `docs/architecture.md` を context menu 新導線へ同期する
...
- [x] N-12.86 実装: Settings のキャラクター設定にモーションデバッグ操作（固定モーション選択 / トリガー再生）を追加し、`emit_character_motion_debug` で `watcher` / `watcher-debug` へ即時配信する
- [x] N-12.86 テスト: `apps/orchestrator/integration.test.js` に UI要素・イベント購読・Rustコマンド配線の回帰検知を追加し、`npm test -w apps/orchestrator -- --test-reporter=spec` で確認する
- [x] N-12.86 文書: `docs/OVERVIEW.md` / `docs/concept.md` / `docs/spec.md` / `docs/architecture.md` をモーションデバッグ導線に同期する
- [x] N-12.87 実装: モード別の表現を具体化し、`neutral=軽い微笑み` / `waiting=あくび` / `need_user=手を振る` / `processing=考え中（腕組み+顎に手）` を prototype/VRM モーションへ反映する
- [x] N-12.87 テスト: `apps/orchestrator/integration.test.js` にモーション適用関数（prototype/VRM）と状態別反映ロジックの回帰検知を追加する
- [x] N-12.87 文書: `docs/OVERVIEW.md` / `docs/concept.md` / `docs/spec.md` / `docs/architecture.md` にモード別表現の既定割り当てを追記する
- [x] N-12.88 実装: Settings に状態別 VRMA 取り込みUI（`neutral/processing/waiting/need-user/completion/error-alert`）を追加し、選択ファイルを `import_character_motion_vrma` で `character-packs/<pack_id>/motions/<state>.vrma` へ保存できるようにする
- [x] N-12.88 実装: Watcher 3D は `pack.motions[state]` の VRMA 再生を優先し、未設定/失敗時は既存の手続きモーションへフォールバックする
- [x] N-12.88 文書: `docs/OVERVIEW.md` / `docs/spec.md` に状態別VRMA取り込みとファイル配置・再生仕様を追記する
- [x] N-12.89 実装: `.vrma` 取り込み時に `character_motion_default_paths[state]` を更新し、選択中Packにモーション未設定でもデフォルトモーションを再利用できるようにする
- [x] N-12.89 テスト/文書: `apps/orchestrator/integration.test.js` にデフォルトモーション設定・再生フォールバックの配線検知を追加し、`docs/OVERVIEW.md` / `docs/spec.md` を同期する
