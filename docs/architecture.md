...
- Orchestrator: Rust（Session/Hook/HookNormalizer/HookStateProjector/IPC）
- Worker: Rust（ConPTY で PTY を実行、Windows では余分なコンソールを出さない）
- Protocol: NDJSON
- Storage: settings.json
- Debug: JSONLログ（`status_debug_events.jsonl` / `subworker_debug_events.jsonl` / `subworker_io_events.jsonl` / `project-prompt-history/*.jsonl`）+ worker_smoke.log
...
- 設定画面はカテゴリタブ（`外観 / Windows / AI / 通知 / キャラクター`）で構成し、アクティブカテゴリだけを表示して縦長画面でも見通しを保つ
- 設定画面のレスポンシブは「アクティブタブ内で十分な幅なら 2 列、狭幅では 1 列」に固定し、項目幅を潰さない
...
- Watcher の 3D描画は CDN 依存を複数候補（esm.sh / jsdelivr）で初期化し、選択中VRMが未設定または読込失敗時は組み込み3D Pack / 機能実証用 3Dプロトタイプモデルへフォールバックする
...
- Settings のモーションデバッグ操作（固定モーション選択 / トリガー再生）は `emit_character_motion_debug` コマンドで `watcher` / `watcher-debug` へイベント配信し、3D モーションへ即時反映する
- プロトタイプ表現の既定割り当ては `neutral=軽い微笑み`、`waiting=あくび`、`need_user=手を振る`、`processing=考え中（腕組み+顎に手）` とする
- 透明 watcher window の選択フレーム制御は通常 watcher / `watcher-debug` で分ける。通常 watcher は focus 中だけ native titlebar / 枠を表示し、blur で外す。`watcher-debug` は既存どおり pointer/focus を使った選択状態制御を行う
- watcher 系IPCの発火条件は最小化する。`watcher_window_ready` は初期化時1回、`set_watcher_window_framed` は通常 watcher では focus/blur 時のみ、`watcher-debug` では選択状態遷移時のみ許可する。pointer enter/leave は `watcher-debug` の選択状態更新として扱い、pointer down は focus 通知欠落を補う 1 回のみ再同期を許可する
- `set_watcher_window_framed` は Frontend 側で直列化し、in-flight 中は同時実行しない（pending は最新値へ上書き）
- Watcher 3D は `loadWatcher3dModel` の同一路径読込を in-flight Promise で畳み、重複ロードを防ぐ。描画は pixel ratio 上限とフレーム間引きで GPU 負荷を抑える
- 起動直後は settings hydration 完了まで通常 watcher の表示を保留し、2D を挟まず `is-3d-loading -> is-3d` で描画を切り替える
- watcher には準備中専用レイヤ（Div）を持たせ、settings hydration 中および 3D 再読込中は `準備中...` を表示する
...
- 通常 watcher は設定上も実装上も 3D 専用とし、選択中VRMが無い場合は組み込み3D Pack またはプロトタイプ3Dを使う。`watcher-debug` は大きめ 3D プレビュー用途として併用する
- Windows の通常 watcher は `shadow=false` と DWM 属性（border none / no round corner / non-client rendering 抑止）を適用し、透明ウィンドウの境界線や影を極力出さない
- 通常 watcher は focus 中だけ Windows の native titlebar / 枠を表示する通常ウィンドウとして開き、移動・最大化・閉じる・サイズ変更は OS 標準操作へ委ねる。blur 時は枠なしへ戻す。`watcher-debug` だけは既存どおり選択状態に応じた frame 制御対象として残す
- 通常 watcher の 3D 表示は window client area に追従して描画し、titlebar / 枠の non-client 操作を妨げない
...
- terminal window close の正本は Rust 側 native window event（`CloseRequested` / `Destroyed`）とし、`label -> session_id` を逆引きして session/worker cleanup と `--exit-on-last-terminal` 判定を完了する。frontend `beforeunload` の `stop_terminal_session` は補助経路としてのみ残す
...
| concept | 実装モジュール | 責務 |
|---|---|---|
| F-1 TerminalTransport | `apps/orchestrator/src/index.html` + PTY worker | PTY 入出力の表示と入力転送 |
...
| F-3 HookStateProjector | `apps/orchestrator/src/agent_event_observer.js` | hook 完了を UI 状態へ投影 |
...
- `set_watcher_window_framed(framed, ipc_session_id)`（通常 watcher は focus/blur、`watcher-debug` は選択状態に合わせて native frame を切り替える）
...
- `terminal-exit { session_id, exit_code }`（表示用。状態確定には使わない）
- `terminal-error { session_id, message }`（表示用。状態確定には使わない）
- `completion-hook-state { source, kind, source_session_id?, state, summary? }`
...
## 3.3 Core Modules
### TerminalTransport
- 入力: PTY `output` / UI `input`
- 出力: Terminal 表示更新 / PTY への入力転送
...
### HookStateProjector
- 入力: hook payload
- 出力: projected state（`success|need_input|failure|idle`）

### SubWorkerCoordinator
- 入力: `session_id, merged_state, hook_complete_event, hook_complete_source, subworker_mode, llm_tool, terminal_context`
...
- 制約: 起動条件はモードで分けず `hook-complete(success|need_input|failure)` に統一し、モード差は最終適用（自動入力するか）だけで扱う
- 制約: 判定/実行は `session_id` ごとに独立させ、ターミナル間の優先順位競合を持たない
- 制約: `paused=true` のセッションでは `noop`、`skip_once=true` は 1 回だけ `noop` 後に解除する
- 制約: `hook-complete` が `success|need_input|failure` でサブワーカー有効時は、同一サイクルで `start` または `skip(理由付き)` を必ず記録し、未記録放置を禁止する
- 制約: hook の再送や重複到達でも、同一文脈（state/reason/最終入力/最終出力）は 1 回に畳み、重複 `llm-start` を抑止する（`hook_complete_source` は dedup 署名に含めない）
...
- 制約: `Esc` で `manual-hold` に入ったターンは hook/SubWorker を停止し、表示ステータスを `idle` としてユーザー入力完了待ちにする。ユーザー `Enter` 確定まで再起動しない（in-flight の結果は適用せず破棄）
- 記録: `mode/confidence/action/result/reason` を `subworker-decision` として emit する

### HookCompletionNormalizer
- 入力: `completion-hook-state`
- 出力: `{ event: 'hook-complete', source: 'hook', state, reason }`
- 制約: `state` は `success|need_input|failure` の完了状態のみを通す
- 目的: サブワーカー起動条件を単一化し、hook payload をそのまま UI 状態へ写像する

### StatusPresentation / Routing
- 状態は `idle/need_input/success/failure` を **区別して保持**する
- 表示/報告用ステータスは `idle/subworker-running/need_input/success/failure` を使う
- 表示/報告用ステータスの優先順位は `subworker-running` > `need_input/success/failure/idle` とする
...
- UI の色は `idle/success=黒`、`need_input=オレンジ`、`failure=赤` で固定し、SubWorker 稼働中だけ緑オーバーレイを重ねる
- **処理ルートは状態ごとに分離**する（アイコン/通知/音/ログなどの出し分けは state によって決める）
- 状態遷移の正本は `docs/spec.md` の hook-only 遷移を参照する
- Codex / ClaudeCode の完了確定は hook を正本とし、prompt marker / terminal idle / terminal exit / input 送信 / プロセス生存判定を状態確定ソースに使わない
- SubWorker 稼働中は状態色レイヤーの上に緑の稼働オーバーレイ（`サブワーカー処理中（Escで抜けます）`）を重ね、稼働終了時に解除する
- SubWorker 稼働中は表示ステータスを `subworker-running` に切り替え、未稼働時は直前 hook の `idle/need_input/success/failure` を維持する
...
- `subworker_prompt_template_markdown` の編集 UI は settings カード内の直接 textarea ではなく、プレビュー + modal editor で扱う。カードには短いプレビューと `編集` / `既定に戻す` を置き、保存確定は modal 側の `保存` でのみ行う
...
- SubWorker の起動条件は論理イベント `hook-complete` のみとし、`source=hook` の完了だけを扱う
- 「操作が固まった」診断時は `status_debug_events.jsonl` を一次情報とし、`manual-hold` / `hook未到達` / `hook-complete未到達` の3系統で切り分ける
...
- 入力連携: `agentWorkActive` 中は補完プレースホルダを描画しない（PTY本文出力を優先）

### TerminalContextMenu
- 入力: Terminal 本文上の `contextmenu`（右クリック）と Settings 画面の非入力領域の `contextmenu`
- 出力: フロントエンド描画の軽量メニュー
- 項目: Terminal では `Open New Terminal / Copy / Paste / Select All / Open Settings`、Settings では `Open New Terminal`
- 制約: terminal 領域と Settings の非入力領域に限定してブラウザ既定メニューを抑止し、入力欄や settings UI の操作部品では既定メニューを残す
- 連携: `Open Settings` は `open_settings_window` command を呼び、terminal-only window でも Settings を別 window で開けるようにする
- 連携: `Open New Terminal` は `open_terminal_window_offset_from_current` command を呼び、選択中/フォーカス中の terminal を優先しつつ、見つからない場合は現在 window から少し右下へずらした位置に開く

### CompletionHook
- `start(on_event)` / `stop()`
- `source_session_id` を伝播して関連付け（PTY セッションと hook を結びつける）
- Codex notify 設定は `~/.codex/config.toml` の **トップレベル**に置き、Windows では `.cmd` 名解決へ依存しないよう `notify = ["node", "<path>/nagomi_codex_notify.js"]` を正本とする
...
#4. データフロー
1) PTY output / input → TerminalTransport → Terminal 表示
2) Hook → CompletionHook → AgentEventObserver
3) hook completed/error/need_input → HookCompletionNormalizer（`hook-complete` + source）
4) HookStateProjector → UI（Watcher/tint）
6) UI は state-to-color マップ（`idle/success=黒`、`need_input=オレンジ`、`failure=赤`、`subworker-running=緑オーバーレイ`）を単一経路で適用する
7) `hook-complete` + state + mode + selected AI Coding Agent → SubWorkerCoordinator → `delegate_input` or `show_advice` or `skip`
...
11) デバッグ表示は `open_character_debug_watcher` で `watcher-debug` を開き、通常 Watcher と同じ `terminal-aggregate-state` を受信して状態同期する。native frame は通常 watcher では focus/blur、`watcher-debug` では pointer/focus を基準に `set_watcher_window_framed` で制御する。終了操作は `close_character_debug_watcher`（UIボタン）またはタイトルバー `×` で行う

#5. ストレージ
- `settings.json`: 通知/AI Coding Agent/Terminal 設定を保存
...
- project 別プロンプト履歴: `AppData/Roaming/com.kitfactory.nagomi/project-prompt-history/<project-key>.jsonl`

#6. Settings
- `llm_tool`
- `terminal_*`（font/size/theme/scrollback/copy）
- `terminal_theme_palette`（8テーマの palette 値。UIは単一テーマ選択）
- `terminal_shell_kind`（`cmd` / `powershell` / `pwsh` / `wsl`）
...
- `terminal_shell_kind=pwsh` -> `pwsh.exe`（未導入時は `powershell.exe` へフォールバック）
...
- 対話中の Enter は入力転送のみを行い、状態は変えない

#8. 拡張性
- CompletionHook を tool ごとに差し替え可能
- AI Coding Agent の tool を選択可能
- `terminal_shell_kind`（`cmd` / `powershell` / `pwsh` / `wsl`）で Windows の terminal 起動コマンドを切替可能。UI では `pwsh.exe` 検出時のみ `PowerShell 7` を表示する

#9. セキュリティ
- ログマスク規則は `docs/spec.md` に従う
- サブワーカーOFF の場合は外部送信しない

#10. 観測/デバッグ
- `terminal-output-broadcast`（任意）
- `status_debug_events.jsonl`（state/hook/subworker/watcher 系）
- `subworker_debug_events.jsonl`（mode/confidence/action/result/reason）
- `subworker_io_events.jsonl`（subworker 入出力）
- `project-prompt-history/<project-key>.jsonl`（hook 完了時の `input_messages` / `last_assistant_message` / `cwd` / `state`）
